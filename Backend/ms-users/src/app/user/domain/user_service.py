...
class UserService:
    ...
    def disable_user(self, user_id: int) -> Dict[str, Any]:
        ...
        if not user['is_active']:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Usuario ya esta deshabilitado"
            )
    ...
    def enable_user(self, user_id: int) -> Dict[str, Any]:
        ...
        if user['is_active']:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Usuario ya esta habilitado"
            )
    ...
    def change_password(self, user_id: int, request: ChangePasswordRequest) -> Dict[str, Any]:
        ...
        if not user['is_active']:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="No se puede cambiar contraseña para usuario deshabilitado"
            )
