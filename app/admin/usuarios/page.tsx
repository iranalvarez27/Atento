...
export default function AdminUsuariosPage() {
  ...
  // Grupo: nombre, supervisor seleccionado y lista de agentes asignados manualmente
  const supervisors = mockUsers.filter((u) => u.role === "supervisor");
  const agents = mockUsers.filter((u) => u.role === "agente");

  const [newGroup, setNewGroup] = useState<{
    name: string;
    supervisorId: string;     // id de supervisor
    memberIds: string[];       // ids de agentes seleccionados
    memberSearch: string;      // búsqueda en la lista de agentes
  }>({
    name: "",
  ...
  function submitCreateUser(e: FormEvent) {
    e.preventDefault();
    // Aquí iría tu lógica real (API). Por ahora, solo cierra y limpia.
  ...
  function submitCreateGroup(e: FormEvent) {
    ...
    const payload = {
      ...
      membersCount: newGroup.memberIds.length, // se calcula solo
    };
    console.log("Crear grupo", payload);
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Administrar usuarios y roles del sistema</p>
          </div>
        </div>

        {/* KPIs y botones a la derecha */}
        <div className="grid items-center gap-4 md:grid-cols-[minmax(0,1fr)_220px]">
          ...
          {/* Botones verticales (abren popups con formularios) */}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {/* Grupos recientes */}
          <Card className="rounded-[24px] bg-[#E7EFF1] shadow-md border">
